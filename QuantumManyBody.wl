...
The output is given as {hamiltonianSpinChainUnitaries , hamiltonianSpinChain}. hamiltonianSpinChainUnitaries contains each unitary term of the decomposition in unitaries {{\!\(\*SuperscriptBox[SubscriptBox[\(U\), \(1\)], \(x\)]\) , \!\(\*SuperscriptBox[SubscriptBox[\(U\), \(1\)], \(y\)]\) , \!\(\*SuperscriptBox[SubscriptBox[\(U\), \(1\)], \(z\)]\)}, ... , {\!\(\*SuperscriptBox[SubscriptBox[\(U\), \(L\)], \(x\)]\) , \!\(\*SuperscriptBox[SubscriptBox[\(U\), \(L\)], \(y\)]\) , \!\(\*SuperscriptBox[SubscriptBox[\(U\), \(L\)], \(z\)]\)}}, with L = Length @ listCouplings.
hamiltonianSpinChain is the total Hamiltonian: hamiltonianSpinChain = Total @ Flatten[hamiltonianSpinChainUnitaries * Last @ listGraphCoefficients , 1]."
